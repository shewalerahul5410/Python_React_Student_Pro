...
def validate_options(options):
    ...
    if kwcase not in [None, "upper", "lower", "capitalize"]:
        raise SQLParseError("Invalid value for keyword_case: " f"{kwcase!r}")

    idcase = options.get("identifier_case")
    if idcase not in [None, "upper", "lower", "capitalize"]:
        raise SQLParseError("Invalid value for identifier_case: " f"{idcase!r}")

    ofrmt = options.get("output_format")
    if ofrmt not in [None, "sql", "python", "php"]:
        raise SQLParseError("Unknown output format: " f"{ofrmt!r}")

    strip_comments = options.get("strip_comments", False)
    if strip_comments not in [True, False]:
        raise SQLParseError("Invalid value for strip_comments: " f"{strip_comments!r}")
    ...
    if strip_ws not in [True, False]:
        raise SQLParseError("Invalid value for strip_whitespace: " f"{strip_ws!r}")
    ...
    if truncate_strings is not None:
        try:
            ...
            raise SQLParseError(
                "Invalid value for truncate_strings: " f"{truncate_strings!r}"
            )
    ...
    if indent_columns not in [True, False]:
        raise SQLParseError("Invalid value for indent_columns: " f"{indent_columns!r}")
    elif indent_columns:
    ...
    if reindent not in [True, False]:
        raise SQLParseError("Invalid value for reindent: " f"{reindent!r}")
    elif reindent:
        options["strip_whitespace"] = True

    reindent_aligned = options.get("reindent_aligned", False)
    if reindent_aligned not in [True, False]:
        raise SQLParseError("Invalid value for reindent_aligned: " f"{reindent!r}")
    ...
    if indent_after_first not in [True, False]:
        raise SQLParseError(
            "Invalid value for indent_after_first: " f"{indent_after_first!r}"
        )
    options["indent_after_first"] = indent_after_first

    indent_tabs = options.get("indent_tabs", False)
    if indent_tabs not in [True, False]:
        raise SQLParseError("Invalid value for indent_tabs: " f"{indent_tabs!r}")
    elif indent_tabs:
        options["indent_char"] = "\t"
    else:
        options["indent_char"] = " "
